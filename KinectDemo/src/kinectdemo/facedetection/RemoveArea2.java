...
public class RemoveArea2 {
	Integer contador;
	HashMap<Integer, List<Point>> pontos;
	HashMap<Integer, HashMap<Integer, List<Point>>> rotulos = new HashMap<Integer, HashMap<Integer, List<Point>>>();
		...
		for (int x = 0; x < img.getWidth(); x++) {
			for (int y = 0; y < img.getHeight(); y++) {
				if (new Color(img.getRGB(x, y)).equals(Color.WHITE)) {
					contador = 0;
					pontos = new HashMap<Integer, List<Point>>();
					segmentation(img, x, y);
					if (contador < 200) {
						for (final List<Point> p : pontos.values()) {
							for (final Point aux : p) {
								img.setRGB(aux.x, aux.y, Color.BLACK.getRGB());
							}
						}
					} else {
						rotulos.put(cont++, pontos);
					}
				}
			}
		}
	...
	public HashMap<Integer, HashMap<Integer, List<Point>>> getRotulos() {
		return rotulos;
	}
	...
	public void segmentation(BufferedImage img, int x, int y) {
		...
		while (!locations.empty()) {
			...
			if (img.getRGB(aux.x, aux.y) == Color.WHITE.getRGB()) {
				img.setRGB(aux.x, aux.y, Color.RED.getRGB());
				contador++;
				if (pontos.get(aux.y) == null) {
					pontos.put(aux.y, new ArrayList<Point>());
				}

				pontos.get(aux.y).add(aux);
	...
	public void setRotulos(
			HashMap<Integer, HashMap<Integer, List<Point>>> rotulos) {
